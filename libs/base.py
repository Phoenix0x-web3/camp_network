...
from libs.eth_async.data.models import DefaultABIs, Networks, TokenAmount, TxArgs
...
class Base:
    ...
    async def sign_message(self, text: str | None = None, typed_data: dict | None = None, hash: bool = False):
        if text:
        ...
        while True:
            try:
                ...
                if time.time() - start_time > max_wait_time:
                    logger.exception(f"{self.wallet} get unsuccess transaction")
                    return False
    ...
    async def check_nft_balance(self, contract: AsyncContract | Contract, id: int | None = None):
        if id:
            ...
            balance = await module_contract.functions.balanceOf(self.client.account.address, id).call()
        else:
            module_contract = self.client.w3.eth.contract(
                address=self.client.w3.to_checksum_address(contract.address), abi=DefaultABIs.Token
            )
    ...
    async def execute_transaction(
        ...
        while attempt <= retry_count:
            try:
                logger.info(f"{self.wallet} Executing {activity_type} transaction{f' (attempt {attempt + 1})' if attempt > 0 else ''}")
                # Send transaction
                tx = await self.client.transactions.sign_and_send(tx_params=tx_params)
                ...
                if receipt and tx.params:
                    ...
                    logger.success(f"{self.wallet} transaction confirmed: {tx.hash.hex() if tx.hash else 0}")
