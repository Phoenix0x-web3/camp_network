...
class CoNFT(Base):
    ...
    async def authorize(self):
        ...
        sign_typed_data = {
            "types": {"EIP712Domain": [], "Message": [{"name": "text", "type": "string"}, {"name": "nonce", "type": "string"}]},
            "primaryType": "Message",
            "domain": {},
            "message": {
    ...
    async def mint(self):
        ...
        args = TxArgs(points=mint_data["points"], signature=mint_data["signature"], tokenUri=token_uri)
        data = contract.encode_abi("mint", args=(args.tuple()))
        tx_params = TxParams(to=Contracts.CONFT.address, data=data, value=mint_amount.Wei)
    ...
    async def get_sign_nonce(self):
        response = await self.browser.get(
            url=f"https://conft.app/connect?address={self.client.account.address.lower()}", headers=self.session_headers
        )
        data = response.json()
