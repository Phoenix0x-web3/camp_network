...
class ResourceManager:
    ...
    async def replace_proxy(self, private_key: str) -> Tuple[bool, str]:
        ...
        success = replace_bad_proxy(private_key, new_proxy)
    ...
    async def replace_twitter(self, private_key: str) -> Tuple[bool, str]:
        """
    ...
    async def mark_proxy_as_bad(self, private_key: str) -> bool:
        ...
        Returns:
            Success status
        """
        return mark_proxy_as_bad(private_key)

    async def mark_twitter_as_bad(self, private_key: str) -> bool:
        """
        ...
        Returns:
            Success status
        """
        return mark_twitter_as_bad(private_key)

    async def mark_wallet_as_blocked(self, private_key: str) -> bool:
        """
        Mark a user's wallet as blocked
        ...
        Returns:
            Success status
        """
        return mark_account_as_blocked(private_key)
    ...
    async def replace_all_bad_proxies(self) -> Tuple[int, int]:
        ...
        for wallet in bad_proxies:
            success, _ = await self.replace_proxy(wallet.private_key)
            if success:
                replaced += 1
    ...
    async def replace_all_bad_twitter(self) -> Tuple[int, int]:
        ...
        for wallet in bad_twitter:
            success, _ = await self.replace_twitter(wallet.private_key)
            if success:
                replaced += 1
